...
{
	uint8_t per;
//	电池电压2.4-3.3V,分压比1/3.3->4000/3.3		
	if(ADvalue>1230)			
	{
		per=10;
	}
...
void tempupdate(uint32_t Tobj,uint8_t Ntc)
{
	...
	if(UnitFlag>0)
	{
		Tobjm=(float)(Tobj*1.8)+320;								//F与℃转换
	}
	else
	{
		Tobjm=Tobj;
	...
	LCD_Clearpart(0,56,240,180,BLACK);
	
	if(Tobjm>999)
	{
		LCD_ShowNum(0,72,i,96,Charasc96,3,0);		//888
		Draw_Circle(151,154,3);						//.
		Draw_Circle(151,154,2);						//.
		Draw_Circle(151,154,1);						//.
		Draw_Circle(151,154,0);						//.
		LCD_ShowNum(158,72,j,96,Charasc96,1,0);		//8
	}
	else
	{
