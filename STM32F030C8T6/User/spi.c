...
void SPI1_Init(void)
{
	...
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource5,GPIO_AF_0);					//PA5复用为 SPI1         
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource6,GPIO_AF_0);					//PA6复用为 SPI1        
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource7,GPIO_AF_0);					//PA7复用为 SPI1 
...
//	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;						//串行同步时钟的第二个跳变沿（上升或下降）数据被采样
	...
	Tempw25=SPI1_ReadWriteByte(0xff);									//启动传输		 
}   
//SPI 速度设置函数
//SpeedSet:
